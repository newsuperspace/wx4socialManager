...
public class SecondLevel implements LevelInterface {
	...
	/*
	 * 层级对象的成员，无需随时都跟随着层级对象返回到前端，需要的时候
	 * 在临时获取然后将Set<Member>返回到前端即可，因此需要添加@JSON
	...
	/*
	 * 前端很少会随着获取层级对象的时候获取权限内容，只会单独获取
	 * 指定层级对象的权限，因此到那个时候再直接向栈顶返回Set<Permission>
	 ...
	 * 如果前端真的需要获取某个层级对象管辖范围内的项目，大可直接将GETTER方法返回的set容器
	 * 放到栈顶扫描成JSON格式字符串传递给前端也不迟。
	 */
	@JSON(serialize=false)
	...
	/*
	 * 待审核项目包含审核通过的、每通过的历史项目、正在审核的项目等
	 * 会随着时间数据变得越来越多，因此这里应该设置JSON注解，防止
	 * struts-json-plugin.jar插件在扫描解析位于值栈顶端的层级化对象
	 * 的时候在这上面花费大多时间，如果前端真的需要获取这部分数据
