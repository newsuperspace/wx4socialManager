...
public class UserAction extends ActionSupport implements ModelDriven<User> {
	...
	public String  getUser4ajax(){
		...
		if(null==u){
			...
			// 修改该用户的qrcode中保存的相对路径 → 拼接成绝对路径url，以此供后端页面上的infoModal对话框的<img src=""/>的src属性使用，以显示该用户的独有qrcode
...
//		String uid  =  this.getModel().getUid();   切记一定要不这样获取数据驱动的请求参数，getModal()是给Struts框架使用的，不是给人使用的★★★★
		String uid = this.user.getUid();
		User u = userService.queryEntityById(uid);
