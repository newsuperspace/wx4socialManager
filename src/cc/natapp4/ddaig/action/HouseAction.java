...
public class HouseAction extends ActionSupport implements ModelDriven<House>{
	...
	/**
	 * AJAX
	 * 获取指定活动室的数据信息，返回给前端用作修改Modal的信息显示
	 * @return
	 */
	...
	public String updateHouse(){
		...
		if(null==h){
			result.setMessage("不存在该活动室，更新失败");
		}else{
			h.setName(house.getName());
			h.setDescription(house.getDescription());
			Geographic  g =  h.getGeographic();
			if(null!=g){
				g.setLatitude(this.latitude);
				g.setLongitude(this.longitude);
			}else{
				g =  new  Geographic();
	...
	/**
	 * AJAX
	 * 关闭房屋使用权,不影响已经预约的活动，但新建活动时不能再选择该房屋
	 * @return
	 */
