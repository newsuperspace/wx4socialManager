...
/**
 * 这里为什么要添加@lazy延迟加载的注解呢？
 * 因为Spring容器（BeanFactory）启动是在web.xml中的Spring监听器
 ...
 * 而是在业务正式开始的时候，在外部访问的时候再创建当前Bean，而那个时候因为整个Web应用已经启动完毕
 * 所以在ServletContext域中的 WxMpInMemoryConfigStorage 也已经存在，自然就不会在创建当前Bean的时候
 * 执行init()初始化方法的时候爆出错误。
 * 
...
//	        .handler(this.kfSessionHandler).end();
	    ...
	    newRouter.rule().async(false).msgType(WxConsts.XML_MSG_EVENT)
	        .event(WxConsts.BUTTON_CLICK).handler(this.menuClickHandler).end();
...
//	    // 消息类型（msgType）是XML_MSG_LOCATION ，对应微信公众平台文档-消息管理-接收消息-接收普通消息 中关于地理信息的location消息类型
